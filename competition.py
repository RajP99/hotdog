...
class Competition(object):
    ...
    def run(self):
        ...
        """
        for name, equation in self.competitors.items():
            elapsed_time = 0
            hotdogs_eaten = 0
            while elapsed_time <= self.duration:
                elapsed_time += equation(hotdogs_eaten)
                hotdogs_eaten += 1
                if elapsed_time <= self.duration:
                    event = Event(elapsed_time, name, hotdogs_eaten)
                    self.events.append(event.rounded())

            partial_hotdog_eaten = (self.duration-self.events[-1].elapsed_time)/(elapsed_time - self.events[-1].elapsed_time)
            self.events.append(Event(self.duration, name, partial_hotdog_eaten).rounded())

        self.events.sort(key = lambda event: (event.elapsed_time, event.name))
    ...
    def winner(self):
        ...
        """
        sorted_by_name = sorted(self.events, key = lambda event: event.total_hot_dogs_eaten)
        return sorted_by_name[0].name
